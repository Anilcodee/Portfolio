...
const BlogCard = ({title, content, date, tags, coverImage, author, views, slug}) => {
  ...
  return (
    <div onClick={() => navigate(`/blog/${slug}`)} className='relative bg-[#1f2933] border border-gray-700 rounded-xl overflow-hidden hover:border-[#00df9a] transition'>
        <img src={coverImage} alt="Blog Cover" className='w-full h-44 object-cover' loading='lazy'/>
        <div className='absolute top-2 left-2 flex gap-2'>
          {
            tags.map((tag) => (
              <span key={tag} className=' border border-gray-600 bg-[#485563] text-gray-300 text-[12px] px-2 py-1 rounded'>
                {tag}
              </span>
            ))
          }
